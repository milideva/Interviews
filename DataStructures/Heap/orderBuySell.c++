...
class Solution {
  ...
  typedef pair<int, int> order; // <price, quantity>
  
  priority_queue <order> buy; //buyOrders maxHeap by price
  priority_queue <order, vector <order>, greater<> > sell; // sellOrders minHeap by price
  
  long moduloNum = pow(10, 9) + 7;
  long buySize = 0;
  long sellSize = 0;
...
public:
  int getNumberOfBacklogOrders(vector<vector<int>>& orders) {
    ...
    return (buySize % moduloNum + sellSize % moduloNum) % moduloNum;
  }
};
...
int main () {
  ...
  auto x = sol.getNumberOfBacklogOrders(orders);
  cout << "getNumberOfBacklogOrders(): " << x << endl;
  return 0;
}
